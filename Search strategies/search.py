# Name: Sowmya Achanta email: omachant@indiana.edu
import time
from heapq import *
import functools
import copy
...
def manhattan_heuristic(node):
    cost = 0
    for current_block in node.blocks:
        (x1,y1,z1) = node.blocks[current_block]
        for goal_block in node.goal:
            (x2,y2,z2) = node.goal[goal_block]
...
def euclidean_heuristic(node):
    ...
    for current_block in node.blocks:
        ...
        for goal_block in node.goal:
            (x2,y2,z2) = node.goal[goal_block]
            if goal_block == current_block:
               cost += math.sqrt(pow((x1 - x2),2) + pow((y1 - y2),2) + pow((z1 - z2),2))
...
def breadthFirst(queue,visited):
    bfs_state_queue = heappop(queue)
    bfs_state_priority = bfs_state_queue[0]
    bfs_state = bfs_state_queue[1]

    #ensure there are no cycles
    visited.append(bfs_state)

    #check if goal is found
    if bfs_state.isGoal():
        print("You've found the solution")
        return queue, bfs_state
    #worlds_in_queue = [v for (k,v) in queue]

    #expand each node and return when no children for a node
    for action in bfs_state.allowedActions():
        neighbor = copy.deepcopy(bfs_state)
        neighbor.do(action)

        if not duplicateWorld(neighbor, [v for (k,v) in queue] ) and not duplicateWorld(neighbor,visited):
            heappush(queue,(bfs_state_priority+1, neighbor))
    return queue, bfs_state

def depthFirst(queue,visited):
    dfs_state_queue = heappop(queue)
    dfs_state_priority = dfs_state_queue[0]
    dfs_state = dfs_state_queue[1]

    visited.append(dfs_state)

    #check if goal is found
    if dfs_state.isGoal():
        print("You've found the solution")
        return queue, dfs_state
    #worlds_in_queue = [v for (k,v) in queue]
    #print worlds_in_queue

    #expand each node and return when no children for a node
    for action in dfs_state.allowedActions():
        #print action
        neighbor = copy.deepcopy(dfs_state)
        neighbor.do(action)

        if not duplicateWorld(neighbor, [v for (k,v) in queue] ) and not duplicateWorld(neighbor,visited):
            heappush(queue,(dfs_state_priority-1, neighbor))
    return queue, dfs_state

def bestFirst(queue,visited,heuristic):
    bestFirst_state_queue = heappop(queue)
    bestFirst_state_priority = bestFirst_state_queue[0]
    bestFirst_state = bestFirst_state_queue[1]

    #add the current state to visited
    visited.append(bestFirst_state )

    #check if it is the goal
    if bestFirst_state.isGoal():
        print("You've found the solution")
        return queue, bestFirst_state
    #worlds_in_queue = [v for (k,v) in queue]

    #check neighbors of the current state
    for action in bestFirst_state.allowedActions():
        neighbor = copy.deepcopy(bestFirst_state)
        deltaCost = neighbor.do(action)

        if duplicateWorld(neighbor,visited):
           continue

        if not duplicateWorld(neighbor, [v for (k,v) in queue]):
...
def aStar(queue,visited,heuristic):
    #get current world from queue
    aStar_state_queue = heappop(queue)
    aStar_state_priority = aStar_state_queue[0]
    aStar_state = aStar_state_queue[1]

    #add the current state to visited
    visited.append(aStar_state)

    #check if it is the goal
    if aStar_state.isGoal():
        print("You've found the solution")
        return queue, aStar_state

    g_score_current = aStar_state.cost
    #check neighbors of the current state
    for action in aStar_state.allowedActions():
        neighbor = copy.deepcopy(aStar_state)
        deltaCost = neighbor.do(action)
        ...
        # deltaCost is the distance between the current state and its neighbor

        g_score_neighbor = neighbor.cost
        cum_g_score = g_score_current + deltaCost

        if not duplicateWorld(neighbor, [v for (k,v) in queue]) or cum_g_score < g_score_neighbor:
                g_score_neighbor = cum_g_score
                f_score = g_score_neighbor+ heuristic(neighbor)
                if not duplicateWorld(neighbor, [v for (k,v) in queue]):
                    heappush(queue,(f_score, neighbor))
...
def run(world,title,heuristics,timeout=10000):
    solutions = []
    queue = []
...
hs = {manhattan_heuristic, euclidean_heuristic} #this is a set, put your heuristic functions in here.
